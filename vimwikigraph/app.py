...
class State:
    ...
    def set_form(self, filter, highlight, collapse):
        self.filter = filter
        self.highlight = highlight
        self.collapse = collapse

    def get_graph(self):
        return copy.deepcopy(self.vimwikigraph)
...
def route_index():
    if request.method == 'GET':
        ...
        rendered = render_template(
            ...
            collapse_value=state.collapse,
        )
        return rendered
    if request.method == 'POST':
        state = State.get_instance()
        state.set_form(request.form['inptFilter'], request.form['inptHighlight'], request.form['inptCollapse'])
        rendered = render_template(
            'index.html',
            filter_value=state.filter,
            highlight_value=state.highlight,
...
def node_json():
    ...
    if 'node' in request.json:
        ...
        #  lines = re.sub(f'{state.filter}', f'<span style="color:red">{state.filter}</span>', lines)
    else:
        lines = []
